...
import { showWarningModal, showConfirmModal } from '../utils.js';
...
export function initTreeBuilder(elementId, dealId, onUpdate) {
    ...
    return render; // Return render function for manual refresh
}

function render() {
    if (!container) return;
    const data = Store.getMapContent(currentDealId);
    ...
    domainKeys.forEach(domainName => {
        ...
        domainEl.className = "bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm transition-all mb-3 hover:shadow-md";
...
function setupSolutionModal() {
    ...
    // Use cloneNode to prevent duplicate listeners if init called multiple times
...
function openSolutionModal(domainName, categoryName, existingSolution = null, index = null) {
    ...
    const shareDisp = document.getElementById('sol-share-disp');

    mInput.value = '';
    nInput.value = '';
    sInput.value = '10';
    shareDisp.innerText = '10%';
    noteInput.value = '';
...
async function fetchPainPoints() {
    ...
    try {
        ...
        // Assuming callGemini returns an object or string. We expect string here or simple structure. 
        
        let painPoints = [];
        let rawContent = "";

        if (typeof resultRaw === 'string') rawContent = resultRaw;
        else if (resultRaw && typeof resultRaw === 'object') {
            if (resultRaw.text) rawContent = resultRaw.text; // fallback
            else if (resultRaw.content) rawContent = resultRaw.content;
            else rawContent = JSON.stringify(resultRaw);
        }

        painPoints = rawContent.split('\n')
            .map(line => line.trim())
...
function renderPainPoints(points, preChecked = false) {
    ...
    points.forEach((point) => {
        ...
        const unselectedClass = "border-gray-200 bg-white text-gray-500 hover:border-gray-300 hover:bg-gray-50";
        const selectedClass = "border-blue-500 bg-blue-50 text-blue-700 shadow-sm ring-1 ring-blue-500/30";
        ...
        const iconUnselected = "border-gray-300 bg-white text-transparent group-hover:border-gray-400";
...
function saveSolutionFromModal() {
    ...
    const share = parseFloat(document.getElementById('sol-share').value);
    const note = document.getElementById('sol-note').value.trim();
    const listContainer = document.getElementById('painpoint-list');
    ...
    if (currentSolutionIndexForModal !== null) {
        result = Store.updateSolution(currentDealId, currentDomainForModal, currentCategoryForModal, currentSolutionIndexForModal, name, share, manufacturer, checkedPainPoints, note);
    } else {
        result = Store.addSolution(currentDealId, currentDomainForModal, currentCategoryForModal, name, share, manufacturer, checkedPainPoints, note);
    }
...
function showAddDomainInput() {
    ...
    const save = () => {
        ...
        if (val) {
            if (Store.addDomain(currentDealId, val)) {
    ...
    const cancel = () => {
        row.remove();
        if (Object.keys(Store.getMapContent(currentDealId)).length === 0) render();
    };
...
function showAddCategoryInput(domainName) {
    ...
    const save = () => {
        const val = input.value.trim();
        if (val) {
            if (Store.addCategory(currentDealId, domainName, val)) {
                render();
                if(refreshCallback) refreshCallback();
            } else {
...
function showEditCategoryInput(domain, oldName, headerEl) {
    ...
    const save = () => {
        ...
        if (newVal && newVal !== originalText) {
             Store.renameCategory(currentDealId, domain, originalText, newVal);
             if(refreshCallback) refreshCallback();
        } 
        render();
    };
...
function showEditDomainInput(domain, headerEl) {
    ...
    const save = () => {
        const newVal = input.value.trim();
        if (newVal && newVal !== originalText) {
             Store.renameDomain(currentDealId, originalText, newVal);
             if(refreshCallback) refreshCallback();
        }
        render();
    };
